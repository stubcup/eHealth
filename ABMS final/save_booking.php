...
if ($result) {
    ...
    if (mysqli_num_rows($result5) > 0) 
    {
        if(mysqli_num_rows($result) > 0)
        {
            ...
            $book_status = $stats['book_status'];

            if ($book_status == '0') {
                echo ("<script>alert('You Have Already Booked This Bus');</script>");
            }elseif ($book_status == '2') {
                $update_query = "UPDATE `booking` SET `book_status` = '0' WHERE `stdNo` = '$stdNo' AND `time` = '$time' AND `bus` = '$bus'  AND `book_date` = '$today'";
                ...
                if ($update_result) { ?>
                    ...
                    <?php echo ("<script>alert('Bus has successfully been rebooked');</script>");
                }
            }
        ...
        while ($row3 = mysqli_fetch_array($result3)) {
            ...
            if ($result2) :
                ...
                $tempDir = 'images/';
                $filename = $today . $time . $bus . $_SESSION['studentID'];
                $body =  $qr_generate;
                $codeContents = $body;
