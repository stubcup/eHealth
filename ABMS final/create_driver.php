...
if (count($errors) ==0) {
    ...
    $queryA="SELECT * FROM administrator WHERE `adminID` ='$idno'";
    $resultA=mysqli_query($db, $queryA);

    if($resultA)
    {
        $queryd = "UPDATE `administrator` SET `email` = '$email',`cell` = '$cell',`passW` = '$passW' WHERE `adminID` = '$idno'";
        $result = mysqli_query($db, $queryd);
...
//admin
    $queryA="SELECT * FROM deriver WHERE `driverID` ='$idno'";
    $resul=mysqli_query($db, $queryA);

    if($resul)
    {
        $queryd = "UPDATE `driver` SET `email` = '$email',`cell` = '$cell',`passW` = '$passW' WHERE `driverID` = '$idno'";
        $result = mysqli_query($db, $queryd);
