...
if (empty($idno) || !is_numeric($idno) || strlen($idno) != 13) {
    ...
    if (getAge($idno) < 15) {
        array_push($errors, "Student is underage restriction. Only Students above the age of 15 can register");
    }else {
...
if (count($errors) == 0) {
    ...
    $query = "INSERT INTO student (`studentID`,`IDNo`,`sname`,`name`, `role`, `createdOn`) VALUES ('$stdNo', '$idno','$sname','$name', '3', '$date')";
    $result = mysqli_query($db, $query);
