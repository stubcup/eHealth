<?php 
	...
	if (isset($_POST['create_appointment'])) {
		...
		if( strlen($id_num)!=13 |!is_numeric($id_num))
                {
                    array_push($errors, "Invalid ID number entered");
                }
                
                if (!ctype_alpha($sname)) {

                array_push($errors, "surname must be alphabetically ");
                 }
               if (!ctype_alpha($name)) {

    array_push($errors, "name must be alphabetically ");
}
                
                 if(is_numeric($sname))
                {
                    array_push($errors, "Surname cant consitst of digits");
                }
                if( strlen($contact)!=10 ||!is_numeric($contact))
                {
                    array_push($errors, "Invalid cellphone numbers ");
                }
                $today=date('Y-m-d');
                if($date<$today)
                {
                    array_push($errors, "You cant book for previous days");
                }
...
if ($result4) {
    ...
    if (mysqli_num_rows($result4) >0) {
        array_push($errors, "Appointment for this time has already been made "); 
    }
}
